...
- ⚠️ **GIAO DIỆN MỚI CHỈ ĐÚNG Ở 16:9 — điện thoại thật là 20:9.** Nhóm điều tra quyết định #37
  tìm ra khi tính hình học ở nhiều tỉ lệ. `CanvasScaler` để `matchWidthOrHeight = 0,5`, nên ở
  máy 1080×2400 (20:9, đa số điện thoại hiện nay) bề ngang hữu dụng **co còn ~966px thiết kế**
  thay vì 1080. Thứ neo trái và thứ neo phải xích lại gần nhau ~114px: banner sự kiện (neo
  trái, hết ở x = 800) **đè lên cột nút** (neo phải, bắt đầu ở x = 718) khoảng **82px**, và
  khung thanh máu bị cắt ra ngoài mép. Không nuốt được cú chạm nào (mọi ảnh giao diện đều
  `raycastTarget = false`), nên đây là lỗi **nhìn**, không phải lỗi bấm. Nhưng đây là lần thứ
  BA cùng một lớp lỗi: chú thích ở `BuildM1Scene` đã tự ghi *"ở 1080x1920 các nút chỉ cách
  vùng chạm 8px; đổi sang màn hình lùn hơn là chúng chồng lên nhau"*. Chưa sửa — cần một phép
  kiểm dựng lại rect ở 4-5 tỉ lệ rồi khẳng định không cặp nào chồng nhau.
