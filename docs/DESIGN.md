...
| 41 | **Bộ soi màn hình — chơi thử chưa bao giờ là công cụ, nhìn pixel mới là** | Ngồi chơi 15 phút mỗi lần sửa giao diện *(loại: chậm, không lặp lại được, và người nhìn quen tay sẽ bỏ sót đúng thứ mình vừa viết)*; chỉ chụp ảnh rồi tự xem *(loại: đã làm suốt M5 và bỏ sót thanh máu đầy nguyên ở 0/100 dù nhìn thẳng vào nó)* | Chủ dự án hỏi có cách nào nhanh hơn chơi thử. Nhìn lại thì **mọi lỗi "phải chơi mới thấy" của dự án này đều ĐO ĐƯỢC**: nút chết bắt bằng bấm qua `EventSystem`, thanh không vơi bắt bằng đo lưới `Image`, nhân vật chìm vào sàn bắt bằng ΔE, banner đè nút bắt bằng hình học — chỉ mỗi việc **nhìn ảnh chụp** là còn thủ công. `SoiManHinh.cs` lái game qua **7 cảnh × 2 tỉ lệ**, mỗi cảnh xuất một PNG kèm bản kê JSON mọi phần tử đang hiện (tên, khung trên màn, chữ, màu, cỡ chữ sau tự co, bề rộng đã vẽ ra, có bị tấm phủ che không, cú chạm vào giữa nút rơi trúng cái gì). `tools/soi-man-hinh.py` đọc cặp đó và báo bốn loại lỗi đo được. **Lần chạy đầu ra 78 lỗi — 72 trong đó là bộ đo sai, và sửa chúng mới là phần việc thật:** (a) camera có `targetTexture` thì `WorldToScreenPoint` đã trả pixel của RT rồi, tôi quy đổi thêm lần nữa nên mọi toạ độ y gấp 1,7 lần; (b) `GetPreferredValues` tính theo cỡ chữ GỐC nên mọi nhãn bật tự co đều bị báo tràn — phải dùng `textBounds` là bề rộng ĐÃ VẼ; (c) chữ nằm sau tấm phủ mờ bị tối đi cùng nền nhưng màu danh nghĩa thì không, so hai thứ đó là báo tương phản thấp khắp nơi; (d) ngưỡng "chữ lớn" lấy theo pixel ảnh chụp thu nhỏ thay vì cỡ người chơi thật sự nhìn. **Sáu lỗi còn lại là THẬT, và hai trong số đó chưa ai biết:** mô tả hàng nâng cấp cần 538 đơn vị trong khung 460 (chính nhãn tôi vừa viết ở #40), và **số tầng + số Mảnh mờ suốt từ M1** — giấy trên gỗ sáng đạt 1,85:1 trong khi quyết định #28 đã đo sẵn rằng chỉ MỰC đạt (7,37:1) và chỉ áp luật đó cho NÚT, không áp cho ba khung thông tin. Sau khi vá: **14 cảnh, 0 lỗi**. Ranh giới được ghi thẳng vào công cụ: nó không biết câu chữ có DỄ HIỂU không và không biết game có VUI không |
...
- ~~⚠️ **GIAO DIỆN MỚI CHỈ ĐÚNG Ở 16:9**~~ — **ĐÃ SỬA**, xem quyết định #39. Và bộ đo dựng ra
  ...
  **mọi** độ phân giải kể cả 16:9, và **đấu trường không lọt khung** ở 20:9 — quái bị cắt
  mất một phần thân.
- **Nút HUD bị tấm phủ chặn khi bảng đang mở.** *(Bộ soi màn hình ở quyết định #41 tìm lại
  đúng mục này một cách độc lập — bốn nút cột phải đều báo "cú chạm trúng Dim" khi bảng mở.
  Bộ soi coi đó là CÓ CHỦ Ý khi có màn hình che toàn bộ đang mở, nên nó không kêu nữa; nhưng
  câu hỏi thiết kế thì vẫn còn nguyên.)* Sau quyết định #36 thì cụm HUD *thông tin*
